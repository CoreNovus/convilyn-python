...
Runs an AI workflow (``goal_lane.content_to_multipost``) end-to-end from a sync
...
Usage:
    ...
    python examples/05_goals_content_to_multipost.py path/to/file_id
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: 05_goals_content_to_multipost.py <file_id>", file=sys.stderr)
        return 1
    file_id = sys.argv[1]

    with Convilyn() as client:
        job = client.goals.run(workflow_id="goal_lane.content_to_multipost", files=[file_id])
